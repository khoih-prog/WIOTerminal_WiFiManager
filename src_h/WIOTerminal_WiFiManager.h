...
const char WM_HTTP_HEAD_END[] PROGMEM =
  "</head><body><div class=\"container\"><div style=\"text-align:left;display:inline-block;min-width:260px;\">";
...
const char WM_HTTP_PORTAL_OPTIONS[] PROGMEM =
  "<form action=\"/wifi\" method=\"get\"><button class=\"btn\">Configuration</button></form><br/><form action=\"/i\" method=\"get\"><button class=\"btn\">Information</button></form><br/><form action=\"/close\" method=\"get\"><button class=\"btn\">Exit Portal</button></form><br/>";
...
const char JSON_ITEM[] PROGMEM = "{\"SSID\":\"{v}\", \"Encryption\":{i}, \"Quality\":\"{r}\"}";
  ...
  "<form method=\"get\" action=\"wifisave\"><fieldset><div><label>SSID</label><input id=\"s\" name=\"s\" length=32 placeholder=\"SSID\"><div></div></div><div><label>Password</label><input id=\"p\" name=\"p\" length=64 placeholder=\"password\"><div></div></div></fieldset>";
  //////
...
const char WM_HTTP_FORM_LABEL_BEFORE[] PROGMEM =
  "<div><label for=\"{i}\">{p}</label><input id=\"{i}\" name=\"{n}\" length={l} placeholder=\"{p}\" value=\"{v}\" {c}><div></div></div>";
const char WM_HTTP_FORM_LABEL_AFTER[] PROGMEM =
  "<div><input id=\"{i}\" name=\"{n}\" length={l} placeholder=\"{p}\" value=\"{v}\" {c}><label for=\"{i}\">{p}</label><div></div></div>";
//////
...
const char WM_HTTP_SAVED[] PROGMEM =
  "<div class=\"msg\"><b>Credentials Saved</b><br>Connecting {v} to the {x} network.<br>Wait around 10 seconds then check <a href=\"/\">if OK.</a> <p/>The {v} AP will be restarted on the radio channel of the {x} network. You may have to manually reconnect to the {v} AP.</div>";
...
#include "WIOTerminal_WiFiManager-Impl.h"
